...
def calc_play(play_info):
    ...
    if len(active_fields.keys()) > 1:
        for field in active_fields:
            ...
            print(field_name)
            if field_cp.field_cps[field_name]['priority'] > priority and field_cp.field_cps[field_name]['min_play'] == 1:
                priority = field_cp.field_cps[field_name]['priority']
        ...
        for field in active_fields.keys():
            field_name = active_fields[field]['name']
            if field_cp.field_cps[field_name]['min_play'] == 1:
